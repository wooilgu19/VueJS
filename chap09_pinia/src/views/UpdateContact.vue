...
const updateContact = async (contact) => {
  try {
    ...
    const resp = await store.updateContact(contact);
    if(resp.status === 'success') {
      Swal.fire({title: 'SUCCESS', text: '데이터 수정 성공', icon: 'success'});
      router.push('/list');   // 목록으로 이동
    } else if (resp.status === 'fail') {
      Swal.fire({title: 'FAIL', text: '데이터 수정 실패', icon: 'warn'})
    }
  } catch(err) {
...
<template>
  <div class="container-fluid">
    ...
    <button class="btn btn-outline-primary" @click="() => updateContact(contact)">UPDATE</button>
  </div>
</template>
